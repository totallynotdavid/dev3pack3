		...
		<div className="container mx-auto px-4 py-8">
			...
			<div className="grid gap-8 lg:grid-cols-2">
				...
				<div className="rounded-lg border border-border bg-card p-6">
					...
					{myOffers.length === 0 ? (
						<p className="text-muted-foreground">No offers made yet. Browse contracts to make an offer.</p>
					) : (
						<div className="space-y-3">
							{myOffers.map((offer) => (
								<div key={offer.id} className="rounded border border-border/50 p-3">
									<div className="flex items-center justify-between">
										...
										{offer.counter_amount && (
											<p className="text-orange-600">Counter: {formatMoney(offer.counter_amount / 100)}</p>
										)}
									</div>
								</div>
							))}
